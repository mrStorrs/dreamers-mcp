...
python3 -m dreamers_stats dashboard --client codex --home /tmp/codex-home --repo current --output /tmp/dreamers-stats.html
...
The `dashboard` command renders a standalone HTML file from the existing bounded reports. Omit `--output` to print the HTML document to stdout.
