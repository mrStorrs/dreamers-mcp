...
The `dashboard` command renders a standalone HTML file from the existing bounded reports. It formats large counts and token totals with commas, shows generated and data-range timestamps in readable UTC text, uses status badges for run groups, and lays out gate and token details in tables. Each run also appears in a clickable details row whose collapsed summary uses the repository basename plus the run's UTC start time, shows that run's token total when one can be attributed safely, and keeps the raw `run_id` and full `repo_path` in the expanded details. Omit `--output` to print the HTML document to stdout. Use `--repo all` for a first smoke test, or run `--repo current` from the repository whose stats you want to inspect. Warning text is spaced for readability, and the dashboard shows `n/a` when a run has no attributable token rows.
